const eyeStyles = {
  ...
  /* --- v3: corner-sealed lids on a symmetric saccade --- */
  v3: `
  :host {
    display: flex;
    align-items: center;
  ...
  /* The crease curve eats into the lid's own corners faster than the
     almond clip-path narrows, leaving a gap at both tips when closed.
     Flat, unrounded end-caps guarantee full coverage there. */
  .eye-lid::before,
  .eye-lid::after {
    content: "";
    position: absolute;
...
class ConceptEye extends HTMLElement {
  ...
  render() {
    ...
    const styles = legacy ? eyeStyles.legacy : eyeStyles.v3;
    const markup = eyeMarkup[version] || eyeMarkup.v3;
