...
`,
  ...
  /* Frame connecting the keyboard deck up to the carriage — the two
     used to float as separate objects. */
  .tw-frame {
    position: absolute;
    left: 10px;
    bottom: 40px;
...
class ConceptTypewriter extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<style>${typewriterStyles[version] || typewriterStyles.v2}</style>${typewriterMarkup[version] || typewriterMarkup.v2}`;
  }
}
