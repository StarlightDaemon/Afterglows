const hourglassStyles = `
  ...
  @keyframes hourglass-upper-drain {
    0%, 12% { transform: translateX(-50%) scaleY(1); opacity: 1; }
    78%, 100% { transform: translateX(-50%) scaleY(0.26); opacity: 0.42; }
  }

  @keyframes hourglass-lower-fill {
    0%, 12% { transform: translateX(-50%) scaleY(0.3); opacity: 0.45; }
    78%, 100% { transform: translateX(-50%) scaleY(1); opacity: 1; }
  }

  @keyframes hourglass-stream-flow {
    0%, 10% { opacity: 0; transform: translateX(-50%) scaleY(0.2); }
    18%, 82% { opacity: 1; transform: translateX(-50%) scaleY(1); }
    90%, 100% { opacity: 0; transform: translateX(-50%) scaleY(0.2); }
  }

  @keyframes hourglass-grain-fall {
    0%, 18% { opacity: 0; transform: translate(-50%, -6px) scale(0.7); }
    26% { opacity: 1; transform: translate(-50%, 0) scale(1); }
    82% { opacity: 1; transform: translate(-50%, 22px) scale(0.9); }
    100% { opacity: 0; transform: translate(-50%, 28px) scale(0.5); }
  }

  @keyframes hourglass-neck-pulse {
...
class ConceptHourglass extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<style>${hourglassStyles}</style>${hourglassMarkup[version] || hourglassMarkup.v2}`;
  }
}
