// v1 is the archived original 3-level tree; v2 deepens the recursion
// to 4 levels.
...
`,
  ...
  /* Level 4: the finest twigs, carrying the leaf tips. Deepens the
     recursion from 3 to 4 levels for a fuller canopy. */
  .ft-seg.l4 {
    bottom: 100%;
    --w: 1px;
    --h: 7px;
...
// The tree is a fixed binary structure (3 levels in the archived v1,
// deepened to 4 levels in v2 per revision request); building the nested
// markup in code keeps the HTML honest about its recursion.
function branch(level, side, maxLevel) {
  if (level > maxLevel) return '';
  const cls = level === 0 ? 'root' : `l${level} ${side}`;
  const leaf = level === maxLevel ? '<div class="ft-leaf"></div>' : '';
  return (
    `<div class="ft-seg ${cls}">` +
      `<div class="ft-bar"></div>` +
      leaf +
      branch(level + 1, 'left', maxLevel) +
      branch(level + 1, 'right', maxLevel) +
    `</div>`
  );
}

function fractalTreeScene(maxLevel) {
  return `
      <div class="ft">
        ${branch(0, '', maxLevel)}
...
const fractalTreeMarkup = {
  v1: fractalTreeScene(3),
  v2: fractalTreeScene(4),
};
...
class ConceptFractalTree extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<style>${fractalTreeStyles[version] || fractalTreeStyles.v2}</style>${fractalTreeMarkup[version] || fractalTreeMarkup.v2}`;
  }
}
