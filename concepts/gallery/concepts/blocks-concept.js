const blocksStyles = `
  ...
  /* Stacking wooden blocks: lettered cubes drop in one at a time to
     build a tower, each landing with a little bounce - until the top
     block overbalances, the tower topples, and the blocks reset. */
  .bk {
    width: 100px;
    height: 96px;
    position: relative;
  ...
  /* Drop-in with a landing bounce; then the whole tower topples. */
  ...
  /* Letters via ::before so they inherit each block's transform. */
  ...
  /* A star sparkle when the third block lands (tower complete). */
  .bk-star {
    position: absolute;
    left: 50%;
    bottom: 82px;
