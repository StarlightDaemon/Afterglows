...
`,
  ...
  /* Air-flow readout. Widened + repositioned - the fixed-width text was
     clipping past the tile edge on "AIR: HI". */
  .bn-read {
    position: absolute;
    left: 50%;
    bottom: 2px;
...
class ConceptBunsen extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<style>${bunsenStyles[version] || bunsenStyles.v2}</style>${bunsenMarkup}`;
  }
}
