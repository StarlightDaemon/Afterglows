...
Both files are linked by root-absolute path (`/assets/js/afterglows-settings.js`, `/assets/css/afterglows-settings.css`) from every page on the live site: the root `index.html`, the concepts gallery (`concepts/header-animation-concepts.html`), `matrix_code/index.html`, `matrix_code/demo_themes.html`, and `retro/crt_demo.html`. There is no per-page theme markup left anywhere — the gear and panel are injected by the script itself, so the control is guaranteed identical everywhere.

`matrix_code`'s three standalone iframe demo pages (`demo_classic.html`, `demo_canvas.html`, `demo_binary.html`) don't carry the gear; they instead load the older, lighter-weight `matrix_code/phosphor_theme.js` purely as a boot-shim to pick up the persisted theme (see `matrix_code/README.md`).
